# Corrosion Detector v8 Final — 実績まとめ & 実環境実装計画

**作成日**: 2026-04-25 (v6 初版)
**確定日**: 2026-04-29 (v8 Final、精度フェーズを区切り、Phase 6 実装フェーズへ移行)
**改訂履歴**:
...
- **v8 Final (2026-04-29)** — **精度フェーズを区切り + Gemini 恒久排除 + Phase 6 移行**:
  - Phase 5.1 採用 (critical_recall=0.900、per_gt_recall=0.244)、5.2 打ち切り (後処理アプローチ無効)、5.3 校正は実施したが本番反映見送り (n=80 不足)
  - **Gemini を本システムから恒久排除**: 検出経路から外す (CODEX P1)、注釈用途も精度向上に寄与せず削除、`HYBRID_SKIP_GEMINI=true` がデフォルト、GEMINI_API_KEY 不要
  - Phase 5.4 LoRA fine-tune は将来課題として保留 (300 枚ラベル + GPU が必要)
  - 直近は Phase 6 (HF Space 再デプロイ + 運用整備) を優先
...
## 1. エグゼクティブサマリー (v8 Final 時点)

| 観点 | v4 (Gemini 単独) | v5/v6 (Hybrid 旧) | **v8 Final (Phase 5.1 採用)** | 改善 |
|------|----------------|------------------|-------------------------------|------|
| critical_recall (画像単位) | 0.400 | 0.800 | **0.900 [0.750, 1.000]** | **+2.25倍** |
| per_gt_recall (領域単位) | 0.032 | 0.154 | **0.244 [0.190, 0.299]** | **+7.6倍** |
| precision@K=30 | 0.267 | 0.333 | **0.367** | +37% |
| F1 @ IoU=0.3 | 0.039 | 0.116 | **0.124** | +3.2倍 |
| 推論時間 (CPU) | 約 8 秒 | 60〜90 秒 | **~30秒 (Gemini OFF)** | (改善) |
| Gemini API コスト | (使用) | (使用) | **$0.00** (検出経路から完全排除) | (削減) |
| デプロイ状態 | ローカルのみ | HF Space + ローカル | **同上 (要 v8 再デプロイ)** | Phase 6 で対応 |
...
### 4.6.2 段階別の構成 (v8 Final、Gemini 恒久排除)

| Phase | 目的 | Gemini | MAX_CLASSIFY | 推定コスト |
|-------|------|--------|--------------|-----------|
| **5.1b** | SAM2 高粒度化の効果を測る | OFF | 40 | **$0.00** |
| **5.2** | 集合体分離 4 手法アブレーション (打ち切り) | OFF | 40 | $0.00 |
| **5.3** | 閾値校正 (画像単位、SAM2 純粋スコア) | OFF | 40 | $0.00 |
| **5.4** | LoRA fine-tune 検証 (将来) | OFF | 40 | $0.00 |
| 5.5 | マルチスケール (任意) | OFF | 40 | $0.00 |
| **合計** | | | | **$0.00** |

**Gemini は v8 Final で恒久排除済**。GEMINI_API_KEY は不要、HYBRID_SKIP_GEMINI=true がデフォルト。
...
### 4.6.4 v8 Final で Gemini を恒久排除した最終判断

検証で以下が確認されたため、**Gemini はシステムから完全に外す**:

- 検出経路に置くと SAM2 の正解候補を消す (CODEX P1 → 修正後も非依存に)
- JSON パース失敗で 25% 候補消失の不安定性
- 閾値校正に使うと LLM 由来不安定性が混入 (CODEX P5)
- subtype 注釈用途も精度向上に寄与せず、API コストの懸念のみ残る

**v8 Final 以降、本システムでは Gemini API を使用しない**。
将来必要になればプロジェクト外で別途 LLM 連携を検討する (例: 検査員レポート補助の独立ツール)。
...
## 7. 完了の定義 (v8 Final 時点での到達状況)

### 精度フェーズ (Phase 5.1〜5.4) — 一旦区切り

- [x] v6 デプロイ完了 (HF Space + ローカル並行)
- [x] **Phase 5.1 採用判定** ✅ critical_recall 0.800 → **0.900**、per_gt_recall 0.154 → **0.244**
- [x] Phase 5.2 集合体分離 — ❌ 打ち切り (後処理アプローチで recall 半減)
- [x] Phase 5.3 閾値校正 — ⏸️ 校正実施したが本番反映見送り (n=80 不足、`weighted` mode が最良 precision 0.273)
- [ ] **Phase 5.4 SAM2 LoRA fine-tune** — 🔜 将来課題、Phase 6 完了後に再開予定
- [ ] release_test 評価 (44 枚) — 🔜 Phase 5.4 完了時に最終評価
- [ ] monitoring_set 累積 100+ 枚 — 🔜 Phase 6 で運用蓄積後

### 実装フェーズ (Phase 6) — 次フォーカス
...
## 8. Phase 6 実環境実装 (v8 Final、現在の最優先フェーズ)

### 8.1 目的

精度フェーズで到達した v8 構成 (critical_recall=0.900) を **本番運用可能な状態に整える**。
精度のさらなる改善 (Phase 5.4 LoRA fine-tune) は実運用データが蓄積してから再開する。

### 8.2 実施項目 (優先順位順)

#### P0 即対応 (今日中)

1. **🚨 GEMINI_API_KEY を完全削除** (恒久排除に伴う始末)
   - Google AI Studio で旧キー (末尾 `...LY`) を **revoke** (運用リスク除去)
   - ローカル `.env` から `GEMINI_API_KEY` 行を削除
   - HF Space → Settings → Variables and secrets で `GEMINI_API_KEY` を **削除**
   - 新規キーは発行不要 (恒久排除のため)

2. **HF Space を v8 Final で再デプロイ**
   - 現在 HF Space は v5 (旧 hybrid_detector + Gemini ON) のまま
   - 修正済ファイル: `Dockerfile.hfspace` (HYBRID_SKIP_GEMINI=true 等)、`README_hfspace.md`、`api/` 一式
   - `scripts/deploy_to_hfspace.ps1` を実行 (api/, public/, Dockerfile 等が同期される)
   - HF Space 再ビルド完了 (約 15 分) 後、`/api/health` で稼働確認
   - 環境変数の effective_env 確認で `HYBRID_SKIP_GEMINI=true` を確認

#### P1 1 週間以内

3. **ローカル PC 運用ガイドの整備**
   - `docs/deployment_guide.md` を v8 構成に更新
   - `start_server.ps1` を Windows スタートアップに登録 (PC 起動時に自動起動)
   - 検査員向けの簡易マニュアル (画像アップロード → 結果確認) を Markdown で 1 ページ

4. **監視 (簡易版)**
   - HF Space Application Logs で `gemini_call_count=0` が続けば Gemini OFF 維持の証跡
   - `latency_ms` p95 が 90 秒超で警告 → SAM2_MAX_MASKS を一時的に絞る運用判断
   - 週次で `tests/eval/run_eval.py` を val で実行し回帰確認

5. **検査員レビュー UI (manual_review_rate=1.000 への対処)**
   - 現状: 全画像が review_flag=true → 検査員はすべて目視確認
   - 短期対処: フロントで confidence 降順表示、上位 N 件にハイライト
   - 中期対処: Phase 5.3 weighted score を表示し、検査員が threshold をスライダーで調整可能に

#### P2 月内

6. **運用画像の蓄積体制**
   - HF Space で処理した画像を S3 / Drive に保存 (オプトイン)
   - 月次で 50-100 枚を選抜し、Phase 5.4 ラベリング候補プールに追加
   - 6 ヶ月で 300-600 枚到達を目安

7. **Cloud Run / GPU 検討タイミング**
   - HF Space CPU 30 秒/画像で十分なら据え置き
   - 業務利用で「もっと早くしたい」要望が出たら Cloud Run T4 ($0.40/h) 移行
   - GPU なら 5-10 秒/画像、Phase 5.4 fine-tune も同環境で

### 8.3 完了基準 (Phase 6)

- [x] v8 hybrid_detector が HF Space で稼働
- [ ] GEMINI_API_KEY ローテーション完了
- [ ] HF Space `/api/health` が v8 構成 (effective_env で確認可) で稼働
- [ ] ローカル PC で start_server.ps1 自動起動
- [ ] `docs/deployment_guide.md` v8 化
- [ ] 検査員レビュー UI で confidence 降順 + 上位 N 件ハイライト
- [ ] 運用 4 週間で重大インシデント (見逃し) なし → 安定稼働宣言

### 8.4 Future Work (Phase 5.4 以降、Phase 6 完了後に再開)

- **Phase 5.4 SAM2 LoRA fine-tune** — 95% 到達の本命
  - 前提: 運用蓄積 + 公開データ拡充で 300 枚ラベル達成
  - GPU 環境 (Runpod A100 $4-8 or Colab Pro+) 確保
  - per_gt_recall 0.244 → 0.50+、critical_recall 0.900 → 0.95+ を期待
- **Phase 5.5 マルチスケール推論** — 5.4 で 95% 未達なら追加検討
- **Phase 5.3 本格再校正** — 300 枚ラベル後に weighted モードで再実行、本番閾値反映

---

## 9. v8 Final 1 行サマリー

> **「Gemini を検出から外し、SAM2 を生かす設計に転換した結果、val critical_recall=0.900・per_gt_recall=0.244 を Gemini コスト $0 で達成。実運用で 4 週間の安定稼働を確認した後、データを溜めて Phase 5.4 LoRA fine-tune で 95% を狙う。」**
