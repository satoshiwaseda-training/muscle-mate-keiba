# Corrosion Depth Estimation — 深さ推定計画書 v2

**作成日**: 2026-04-29 (v1 初版)
**改訂日**: 2026-04-29 (**v2 — CODEX 厳格レビュー全 8 項目反映**)
**プロジェクト**: Corrosion Detector の拡張機能 (Phase 7+)
**前提**: v8 Final で確定した SAM2 検出パイプラインを上流とし、検出された腐食領域に対して「深さ」**または「外観上の深さ傾向」** を推定する
**作成者**: サトシ (オーナー単独承認モード)
...
| Ver. | 日付 | 変更内容 |
|------|------|--------|
| v1 | 2026-04-29 | 初版 — 4 段階アプローチ、v8 Final と統合、CODEX レビュー想定で採用ゲート明示 |
| **v2** | 2026-04-29 | **CODEX 第 1 次レビュー全 8 項目反映**: 深さ定義の分離、Depth Anything の役割降格、ArUco 単独 mm 換算の削除、古典 ML 優先、API 独立モジュール化、評価ハーネス分離、ライセンス精密化 |

---

## v2 の核心 — v1 から変わった 4 つの本質

1. **「深さ」を **2 系統に分離**: `appearance_depth_class` (見た目、UT なし可) と `measured_depth_class` (UT/実測由来のみ、学習・評価の主軸)。 UI には「外観上の深さ傾向」「要確認度」と表示し、**断定的な mm 表示は UT/基準試験片併用時のみ**。
2. **単眼深度モデル (Depth Anything / MiDaS) を「腐食深さ」とは呼ばない**: シーン奥行きと腐食凹みは物理スケールが 100-1000 倍異なる。出力名は `visual_depth_hint` に降格、評価は **UT 最深点との一致率** で測る。
3. **ArUco 単独で絶対 mm 換算しない**: ArUco から得られるのは横方向 mm/px と姿勢のみ、Z 軸 (凹み深さ) は出ない。Phase 7.3 は **UT 校正主軸**、ArUco は補助 (面積・姿勢補正) のみ。
4. **モデル順序の入れ替え**: 古典 ML (LR/LightGBM) → CNN の順。300 枚規模で CNN を先にやると過学習が見えにくい。
...
2. **`tests/eval/datasets/labels_depth_v0.schema.json`** (新規)

```json
{
  "image_id": "ccs_train_42",
  "polygon_id": "p0",                      // 同一画像内の region 識別
  "polygon": [[y, x], ...],
  ...
  "measured_label_uncertainty": "low" | "medium" | "high",

  "depth_mm":               2.3,           // optional, UT/実測のみ
  "depth_mm_method":        "ut_5pt_avg" | "ut_grid_8x8" | "profile_meter",
  "depth_mm_n_points":      5,
  "depth_mm_uncertainty_mm": 0.4,
  "depth_mm_ci95":          [1.6, 3.0],

  "ut_points": [                            // optional
    {"px_y": 245, "px_x": 312, "thickness_mm": 4.7, "remaining_pct": 78.3}
  ],

  "structure_id":           "site_A_pipe_3",  // split リーク防止用
...
3. **重要ルール (凍結)**:
   - UT/実測なしの `depth_mm` 出力は **禁止**
   - UI 文言: UT なしでは「外観上の深さ傾向」「要確認度」、UT ありでのみ「推定減肉量 (mm)」
   - 学習・評価の主ラベルは `measured_depth_class` のみ
...
- **主ゲート**: `measured_depth_class` 付きサンプル (~50-100 枚見込み) のみで macro F1 ≥ 0.70
- 補助: `appearance_depth_class` 全件 (~200 枚) での F1 (上振れの可能性あるので参考値)
- split: **構造物・撮影系列・腐食箇所単位** で train/val を分離 (画像単位はリークするため不採用)
...
#### 評価指標 (CODEX P0-3 反映、目視 5 段階を却下)

| 指標 | 計算法 | ゲート |
|------|-------|------|
| `deepest_point_top1_hit_rate` | UT 最深点が hint top1 ピクセルから半径 r に入る率 | ≥ 0.50 |
| `deepest_point_top3_hit_rate` | UT 最深点が hint top-K (3) 領域に入る率 | ≥ 0.80 |
| `within_region_rank_corr` | UT 複数点 (5+) と hint 値の Spearman 相関 | ρ ≥ 0.5 |
| `failure_rate` | 完全に外している (相関 ≈ 0) 画像の割合 | ≤ 0.20 |
| `latency_p95` | 推論時間 95 パーセンタイル | ≤ 5 秒 (CPU) |
...
`warning` には「UT データ不足」「ArUco 検出失敗」等を入れる。

#### 評価

- ペア (画像 + UT) を構造物単位で **train/test split**
- test での MAE / RMSE / 信頼区間カバレッジ
- 浅腐食 (UT < 2mm) と深腐食 (UT > 5mm) で別計算 (大きさ依存の傾向確認)
...
api/index.py
  └─ _run_v5_detector_pipeline 後段で
     if DEPTH_ESTIMATION_ENABLED:
         depth_results = DepthEstimator().annotate(image, detections)
         # 検出結果を **削除・並べ替え・review_flag に影響させない**
```

### 4.3 責務分離 (CODEX 第 2 次レビュー P4 と整合)

`DepthEstimator.annotate()` は:
- 検出 list を **読むのみ**
- `Detection.meta` に深さ情報を **追記のみ**
- polygon は変更しない、検出を消さない、confidence を書き換えない
...
- 既存 `/api/analyze` レスポンスに `depth` フィールドが含まれない
...
| リスク | 確率 | 影響 | 回避策 |
|-------|------|------|------|
| 推論時間が伸びる (CPU 30s → 60s) | 中 | 中 | Depth Anything V2 small で +1-2s、古典 ML は +0.1s、許容範囲 |
| `depth` フィールドが UI を破壊 | 低 | 中 | `DEPTH_ESTIMATION_ENABLED=false` で完全 OFF、回帰テスト必須 |
| UI で「mm」と誤表示 | 低 | 大 | UT なし時は **mm 表示自体を出さない**、disclaimer 必須 |
...
> **「Depth Anything は腐食深さを返さない、ArUco は Z 軸を出さない、経験則ラベルは深さの証明にならない。だから measured と appearance を分離し、UT 校正のあるサブセットだけで深さ精度を評価する。LLM は今回も使わない。」**
