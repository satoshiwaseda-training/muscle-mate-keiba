...
# v1: initial (entry + training + paddock features)
# v2: added pedigree + camp features (sire, damsire, breeder, owner tiers,
#     distance/surface fit, composites)
SCHEMA_VERSION = 2
...
def extract_structured_features(
    ...
    # ── Pedigree & camp composite scores (v2) ──
    # These depend on both horse-level (sire, breeder) and race-level
    # (distance, surface, track_condition) data, so they are computed
    # after both are extracted.
    race_distance = race_features.get("distance", 0)
    race_surface = race_features.get("surface", "")
    race_track_cond = race_features.get("track_condition", "")

    for name, hf in horse_features.items():
        ped_feats = pf.extract_pedigree_features(
            sire_name=hf.get("sire_name", ""),
            dam_name=hf.get("dam_name", ""),
            damsire_name=hf.get("damsire_name", ""),
            breeder_name=hf.get("breeder_name", ""),
            owner_name=hf.get("owner_name", ""),
            ritto=hf.get("ritto", ""),
            race_distance=race_distance,
            surface=race_surface,
            track_condition=race_track_cond,
        )
        # Merge pedigree features into horse features
        hf.update(ped_feats)
...
def _extract_horse_features(h: dict) -> dict:
    ...
    # ── Pedigree & camp features (v2) ──
    # These are extracted here rather than in a separate enrichment step
    # so they appear in the structured_features snapshot alongside all
    # other pre-race features.
    features["sire_name"] = h.get("sire", "")
    features["dam_name"] = h.get("dam", "")
    features["damsire_name"] = h.get("damsire", "")
    features["breeder_name"] = h.get("breeder", "")
    features["owner_name"] = h.get("owner", "")
