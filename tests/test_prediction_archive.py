...
def test_store_prediction_exports_review_archive():
    ...
    old_journal = plog.JOURNAL_DIR
    old_recovery = plog.RECOVERY_DIR
    old_status = plog.PERSISTENCE_STATUS_FILE
    tmp_path = _workspace_tmp()
    try:
        plog.LIVE_FILE = tmp_path / "live_predictions.json"
        plog.ARCHIVE_DIR = tmp_path / "prediction_archive"
        plog.JOURNAL_DIR = tmp_path / "prediction_journal"
        plog.RECOVERY_DIR = tmp_path / "recovery"
        plog.PERSISTENCE_STATUS_FILE = tmp_path / "persistence_status.json"
        ...
        assert (
            tmp_path / "prediction_journal" / "2026-04-25" / "events.jsonl"
        ).exists()
        assert (tmp_path / "persistence_status.json").exists()
        ...
        plog.JOURNAL_DIR = old_journal
        plog.RECOVERY_DIR = old_recovery
        plog.PERSISTENCE_STATUS_FILE = old_status
