# Corrosion Detector v7 — 実績まとめ & 精度向上 次期計画書

**作成日**: 2026-04-25 (v6 初版)
**改訂履歴**:
- v6 初版 — Phase 5.1〜5.5 ロードマップ
- v6.1 — CODEX レビュー P0/P1/P2/P3 反映 (Step 0 追加)
- **v7 (2026-04-25 夕方)** — **Gemini 段階投入戦略への転換**:
  - Phase 5.1a 実測で Gemini が精度を下げる構造的問題を確認 (詳細: §3.4)
  - Gemini を Phase 5.1/5.2 で完全 OFF、Phase 5.3 以降で最小投入する戦略に変更
  - MAX_CLASSIFY と SAM2_MAX_MASKS の単一ソース化 (環境変数の独立性問題への対処)

**作成者**: サトシ (オーナー単独承認モード)
**目的**:
1. 計画 v1 → v7 の実装実績を 1 ファイルに集約
2. 「critical_recall ≥ 0.95」を狙う次期アクションを優先順位付きで提示
3. **Gemini 投入を段階的に最小化** しつつ Phase 5 を進める

**v7 の核心**:
> 「**Gemini は精度の天井を引き下げる要因**になっており、SAM2 の純粋な性能改善 (Phase 5.1〜5.4) を測定する間は OFF にする。Gemini は Phase 5.3 以降で「分類器」として最小限投入する。」
...
## 4.6 v7 Gemini 段階投入戦略 (新規)

「測りたいもの」と「Gemini が必要なもの」を分離する設計原則。

### 4.6.1 段階別の Gemini 設定

| Phase | 目的 | Gemini | MAX_CLASSIFY | 推定 Gemini call | 推定コスト |
|-------|------|--------|--------------|----------------|-----------|
| **5.1b** | SAM2 高粒度化の効果を測る | OFF | 40 | 0 | **$0.00** |
| **5.2** | 集合体分離 4 手法アブレーション | OFF | 40 | 0 | **$0.00** |
| **5.3** | 閾値校正 (best variant のみ) | ON | **3** | 80 × 3 = 240 | **~$0.07** |
| **5.4** | LoRA fine-tune 検証 (最終 1 回) | ON | **3** | 80 × 3 = 240 | **~$0.07** |
| 5.5 | マルチスケール (任意) | OFF or ON | 3 | 0〜240 | $0〜0.07 |
| **合計** | | | | | **~$0.14** |

Phase 5 全体で **15 セント以下**、初期段階はゼロコストで進行可能。
