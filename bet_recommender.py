"""単勝 + ワイド 買い目レコメンダ (presentation layer — v3.1, 2026-09-19).
...
ワイドは着順ペア順1-2/1-3/2-3=全122R実証済):
  G1 n=46 / G2 n=76。憲法 §4.2 に従い 100 ベット未満は確定視しない (参考値)。
  v3.0: 2026春G1全レース(桜花賞〜宝塚記念)を追加し G1 n=38→46, G2 n=63→71 に更新。
  v3.1: 2026夏G2 5R(札幌記念/紫苑S/セントウルS/セントライト/ローズS)を追加し G2 n=71→76。

STATS は 2026-09-19 実スクリプト出力値をそのまま使用。手書き・推測値は一切含まない。
"""
...
RECOMMENDER_VERSION = "bet-recommender-v3.1-tansho-wide-2026-09-19"
T = 100  # 1点 100円
MARKS = ["◎", "○", "▲"]


# ── 検証済み実績テーブル ────────────────────────────────────────────
# 出所: tools/analyze_tansho_wide_portfolio.py 実行結果 (2026-09-19)
# finishing_order 重複除去済。ワイドは着順ペア順(1-2/1-3/2-3)で名前突合。
# v3.0: 2026春G1(桜花賞〜宝塚記念)+G2 8R 追加 → G1 n=46, G2 n=71。
# v3.1: 2026夏G2 5R 追加 → G2 n=76 (G1 は不変)。
...
STATS: dict = {
    ...
    "G2": {
        "単勝本命のみ":            {"hit": 0.237, "roi": -0.351, "roi_ex2": -0.439, "streak": 12, "n": 76, "tag": "見送り"},
        "単勝本命+ワイド本命対抗":  {"hit": 0.395, "roi": -0.356, "roi_ex2": -0.425, "streak": 7,  "n": 76, "tag": "見送り"},
        "単勝本命+ワイド軸流し":    {"hit": 0.526, "roi": -0.266, "roi_ex2": -0.355, "streak": 7,  "n": 76, "tag": "見送り"},
        "単勝本命×2+ワイドBOX3頭": {"hit": 0.592, "roi": -0.241, "roi_ex2": -0.329, "streak": 6,  "n": 76, "tag": "見送り"},
        "単勝本命+ワイドBOX3頭":   {"hit": 0.592, "roi": -0.213, "roi_ex2": -0.306, "streak": 6,  "n": 76, "tag": "見送り"},
        "ワイドBOX上位3頭":        {"hit": 0.461, "roi": -0.167, "roi_ex2": -0.270, "streak": 6,  "n": 76, "tag": "見送り"},
        "ワイドBOX人気1-4":        {"hit": 0.592, "roi": -0.238, "roi_ex2": -0.335, "streak": 3,  "n": 76, "tag": "見送り"},
    },
}
...
RATIONALE: dict = {
    ...
    "ワイドBOX人気1-4":        "的中率最大(G1=72%/G2=59%)。ただし投資に対しROIが-13〜-24%。娯楽枠。",
}
...
def recommend_bets(ranked: list[dict], grade: str) -> dict:
    ...
    if gk == "G1":
        ...
        rec["verdict"] = "見送り推奨"
        rec["why"] = (
            "G2は単勝・ワイド全構成でROIがマイナス(本命単勝的中率24%・ROI-35%)。"
            "n=76レースで一度も正期待値は確認されていない。投資対象外として見送りが正解。"
            "どうしても参加するなら、最も傷が浅いワイドBOX上位3頭(ROI-17%、300円)に限定。"
        )
        out["recommendation"] = rec
        out["alternatives"] = [
            combos["ワイドBOX人気1-4"],  # 的中率最大 59.2% (ただしROI-24%)
            combos["単勝本命のみ"],      # 最低コスト100円 (ROI-35%)
        ]
        out["note"] = (
            "G2は n=76 で投資水準に一度も達していません。"
